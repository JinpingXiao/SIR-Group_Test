...
class SGD_Momentum:
    def __init__(self, parameters, lr = 0.01, beta = 0.9, weight_decay = 0.01, nesterov = True):
       ...
       self.lr = lr # 学习率，参数更新方程中的eta
       self.beta = beta # 动量累积方程得到的ß值
       self.weight_decay = weight_decay # L2正则化系数
       self.velocities = {id(p): torch.zeros_like(p.data) for p in self.parameters} # 存储每一轮训练后的“速度”, 下一轮训练后取出
    ...
    def step(self):
        with torch.no_grad():
            for p in self.parameters:
                if p.grad is None:
                    continue
                d_p = p.grad.data # 参数的梯度值
                if self.weight_decay != 0:
                    d_p = d_p + self.weight_decay * p.data # L2正则化，强行在梯度后加上一部分权重本身的大小
